...
public abstract partial class ContextlessPlugin : EditorPlugin
{
	...
	public sealed override void _Input(InputEvent @event)
	{
		...
		if (viewport != null && !GetTree().Root.IsInputHandled())
		{
			...
			if (@event.GetPropertyList().Any((d=>d["name"].AsString() == "position")))
			{
				...
				justifiedEvent.Set("position", justifiedEvent.Get("position").AsVector2() - viewport.GetScreenTransform().Origin);
			}
	...
	{
		bool terrain3Dpresent = GetEditorInterface().IsPluginEnabled("terrain_3d");
		...
		var hsplitContainer2 = terrain3Dpresent ? hsplitContainer1.GetChild(1) : hsplitContainer1.GetChild(0);
