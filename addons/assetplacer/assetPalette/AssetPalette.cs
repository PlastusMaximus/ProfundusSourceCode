...
public partial class AssetPalette : Node
{
	...
	{
		var packedSceneResource = ResourceLoader.Load<Resource>(path);
		if (packedSceneResource is PackedScene packedScene)
		{
			var instantiateOrNull = packedScene.InstantiateOrNull<Node3D>();
			if (instantiateOrNull == null) return false;
			instantiateOrNull.QueueFree();
			return true;
		}
		
		GD.PrintErr($"{nameof(AssetPlacerPlugin)}: Failed to add Resource {path}. Only Scenes can be added (.tscn files or 3D files imported as scene). Maybe the file was moved or deleted.");
		return false;
	}
	...
	private bool IsFileFormatValid(string filePath)
	{
		...
		string[] validEndings =
		{
			sceneFileEnding, objFileEnding, gltfFileEnding, glbFileEnding, fbxFileEnding, colladaFileEnding, blendFileEnding
		};

		// Check File Ending
		return validEndings.Any(filePath.EndsWith);
	}
	...
	private void OnAddAssetData(IEnumerable<Asset3DData> assets)
	{
		...
		foreach (var asset in assets)
		{
			...
			{
				if (IsFileFormatValid(asset.path))
				{
				...
				{
					GD.PrintErr($"{nameof(AssetPlacerPlugin)}: {asset.path} is not a valid 3D asset");
				}
			}
		}
	...
	private void UpdateAssets(IEnumerable<Asset3DData> newAssets, Asset3DData.PreviewPerspective perspective = Asset3DData.PreviewPerspective.Default, int scrollPosition = 0)
	{
		...
		{
			DeselectAsset();
			_assetPlacerUi.UpdateAllAssets(new List<string>());
			return;
		}
		if (newAssets == null)
		{
			DeselectAsset();
			_assetPlacerUi.UpdateAllAssets(CurrentLibraryData.GetAssetPaths(), scrollPosition);
			newAssets = CurrentLibraryData.assetData;
		}
		else
		{
			_assetPlacerUi.AddAssets(newAssets.Select(a=>a.path), scrollPosition);
		}
	...
	private void GeneratePreviews(IEnumerable<Asset3DData> libraryData, bool forceReload, Asset3DData.PreviewPerspective libraryPerspective = Asset3DData.PreviewPerspective.Default)
	{
		...
		{
			if (PreviewGenerator.HandledFiles().Any((f) => asset.path.EndsWith(f)))
			{
	...
	private Node3D CreateHologram()
	{
		...
		var hologram = scene.Instantiate<Node3D>();
		return hologram;
	}
